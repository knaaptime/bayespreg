...
def summarize_spatial_effects(result: pd.DataFrame) -> None:
    """Print a formatted summary table of Bayesian spatial effects.

    Parameters
    ----------
    result : pd.DataFrame
        DataFrame returned by :meth:`spatial_effects`.
    """
    model_label = result.attrs.get("model_type", "unknown")
    G = result.attrs.get("n_draws", "?")

    print(f"Spatial Effects Summary ({model_label} model, {G} posterior draws)")
    print("=" * 76)
    header = (
        f"{'Variable':<16} {'Direct':>10} {'p-val':>8} "
        f"{'Indirect':>10} {'p-val':>8} {'Total':>10} {'p-val':>8}"
    )
    print(header)
    print("-" * 76)
    ...
    for name in result.index:
        ...
        di = row["indirect"]
        t = row["total"]
        dp = row["direct_pvalue"]
        ip = row["indirect_pvalue"]
        tp = row["total_pvalue"]
        print(
            f"{name:<16} {d:>10.4f} {dp:>8.4f} "
            f"{di:>10.4f} {ip:>8.4f} {t:>10.4f} {tp:>8.4f}"
        )

    print("-" * 76)
    print("Bayesian p-values: 2 × min(P(effect > 0), P(effect < 0))")
    print("Credible intervals (95%):")
    for name in result.index:
        row = result.loc[name]
        print(
            f"  {name:<16} D:[{row['direct_ci_lower']:.4f}, {row['direct_ci_upper']:.4f}]  "
            f"I:[{row['indirect_ci_lower']:.4f}, {row['indirect_ci_upper']:.4f}]  "
            f"T:[{row['total_ci_lower']:.4f}, {row['total_ci_upper']:.4f}]"
        )